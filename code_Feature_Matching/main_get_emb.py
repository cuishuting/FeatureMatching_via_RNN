...
class SepLSTMsOneMLP(nn.Module):
    def __init__(self, batch_size, seq_len, temporal_feature_num, hidden_size_lstm, static_dim, static_embeddings_dim, hidden_dim_mlp, targets_dim):
        super(SepLSTMsOneMLP, self).__init__()
    ...
    def forward(self, input_data):
        sample_static_features = input_data[0] # shape: [batch_size, static_features_dim]
        sample_temporal_features = input_data[1] # shape: [batch_size, 24, 12]
        sample_temporal_masks = input_data[2] # shape: [batch_size, 24, 12]
        sample_targets = input_data[3] # shape: [batch_size, 21]
...
def test(test_loader, model, device, targets_type, batch_size):
    ...
    with torch.no_grad():
        for t_id, test_data in enumerate(test_loader):
            ...
            cur_predicts = model(test_data)["predicts"]  # shape: [batch_size, 21]
            cur_targets = test_data[3] # shape: [batch_size, 21]
    ...
    print("var: each_target_pos:\n")
    print(each_target_pos)
    print("test set size:\n")
    print(targets_for_auprc_auroc.shape[1])
...
def main(data_addr, search_space, tune_times, epochs, seq_len, temporal_feature_num, static_feature_num, targets_dim, features_belongs):
    """var "features_belongs" has only 2 unique values: "MV_test", "CV_test" in Get_Train_Test_Data_2.py """
    device = "cpu"
    ...
    dataset_all_features = OrgMIMICIIIDataset(data_dir, features_belongs)
    ...
    train_set, test_set, val_set = random_split(dataset_all_features, [train_size, test_size, val_size], generator=torch.Generator().manual_seed(2147483647))
    final_test_loader = 0
    ...
    for tune_time in range(tune_times):
        ...
        model = SepLSTMsOneMLP(config["batch_size"], seq_len, temporal_feature_num, config["hidden_dim_lstm"], static_feature_num, config["static_embeddings_dim"], config["hidden_dim_mlp"], targets_dim)
        ...
        if (best_val_loss == 0) or (cur_avg_val_loss < best_val_loss):
            ...
            best_config = config
            model_weight_file_path = "/storage1/christopherking/Active/mimic3/c.shuting/MIMICIII_DeepRecurrent_Models/Trained_Models_State/best_model_weights_" + features_belongs + ".pth"
    ...
    best_model = SepLSTMsOneMLP(best_config["batch_size"], seq_len, temporal_feature_num,
    ...
    best_model.load_state_dict(torch.load("/storage1/christopherking/Active/mimic3/c.shuting/MIMICIII_DeepRecurrent_Models/Trained_Models_State/best_model_weights_" + features_belongs + ".pth"))
    test(final_test_loader, best_model, device, targets_dim, best_config["batch_size"])


search_space = {"static_embeddings_dim": list(range(20, 30)),
                "hidden_dim_lstm": list(range(3, 11)),
                "hidden_dim_mlp": [150, 200, 250, 300, 350], # final mlp's(used to predict final target) hidden dim
...
targets_dim = 21
# MV_test
print("Begin training on MV:\n")
data_addr = "/storage1/christopherking/Active/mimic3/c.shuting/MIMICIII_DeepRecurrent_Models/MIMIC_data_new_by_cst/test_imputed_MV_1_24.npz"
temporal_feature_num = 82
static_feature_num = 35
main(data_addr, search_space, tune_times, epochs, seq_len, temporal_feature_num, static_feature_num, targets_dim, "MV_test")

print("Begin training on CV:\n")
data_addr = "/storage1/christopherking/Active/mimic3/c.shuting/MIMICIII_DeepRecurrent_Models/MIMIC_data_new_by_cst/test_imputed_CV_1_24.npz"
temporal_feature_num = 101
static_feature_num = 35
main(data_addr, search_space, tune_times, epochs, seq_len, temporal_feature_num, static_feature_num, targets_dim, "CV_test")
